...
cp "$API" "${API}.bak.directa.$(date +%s)"
echo "✅ Backup: ${API}.bak.directa.$(date +%s)"
...
object ApiSportsRepository {
    ...
    suspend fun resolverFixtureId(
        ...
        DebugLog.log("🔍 Buscando: '$equipoLocal' vs '$equipoVisitante'")
        DebugLog.log("   Fecha: $fechaYYYYMMDD · Hora: $horaEvento · Liga: $ligaHint")
    ...
    private suspend fun buscarPorFechaYNombre(
        ...
        cacheFixtures[cacheKey]?.let { 
            DebugLog.log("📦 Cache: $it")
            return@withContext it 
        }

        val json = httpGet("/fixtures?date=$fecha") ?: run {
            DebugLog.log("❌ No pude traer fixtures del $fecha")
            return@withContext null
        }
        val fixtures = json.optJSONArray("response") ?: return@withContext null
        DebugLog.log("📅 ${fixtures.length()} partidos en la API el $fecha")
        ...
        DebugLog.log("🧪 Normalizado: '$localNorm' vs '$visitanteNorm'")
        ...
        for (i in 0 until fixtures.length()) {
            ...
            DebugLog.log("  📌 '$home vs $away' → score ${"%.2f".format(score)}")
        ...
        if (mejorFid != null && mejorScore >= 1.5) {
            DebugLog.log("✅ MATCH: '$mejorDesc' → $mejorFid (score ${"%.2f".format(mejorScore)})")
            cacheFixtures[cacheKey] = mejorFid
            mejorFid
        } else {
            DebugLog.log("❌ SIN MATCH confiable (mejor: ${"%.2f".format(mejorScore)} '$mejorDesc')")
            null
        }
    }
...
echo "✅ ApiSportsRepository.kt reescrito con búsqueda simplificada + logs extensivos"
